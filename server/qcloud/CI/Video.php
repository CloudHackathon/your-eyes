...
class Video
{
    ...
    public static function upload_slice($filePath, $userid = 0, $title = '', $desc = '', $magicContext = '')
    {
        ...
        if ($rsp['httpcode'] != 200 || $rsp['code'] != 0)//错误
        {
            ...
            if (isset($rsp['data']))//秒传命中，直接返回了url
            {
    ...
    public static function generateResUrl($userid = 0, $fileid = null, $oper = '')
    {
        if ($fileid) {
            if ($oper) {
                return Conf::API_VIDEO_END_POINT . Config::CI_APP_ID . '/' . $userid . '/' . $fileid . '/' . $oper;
            } else {
                return Conf::API_VIDEO_END_POINT . Config::CI_APP_ID . '/' . $userid . '/' . $fileid;
            }
        } else {
            return Conf::API_VIDEO_END_POINT . Config::CI_APP_ID . '/' . $userid;
        }
    }
